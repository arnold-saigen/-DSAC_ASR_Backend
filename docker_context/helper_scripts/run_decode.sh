...
# decode audio with or without diarization
sucesfully_decoded=false
if [ $DIA == "yes" ]; then
   bash ~/saigen/tools/speech_recognition/decode_wav_with_diarization.sh \
     --num_speakers $NUM_SPK \
     --sr $SR \
     --num-channels $NUM_CHAN \
     --num-threads $NUM_THREADS \
     --tag $TAG \
     --dir-work $dir_work \
     $dir_models/$model \
     $dir_models/$model/exp/chain_cleaned/tdnn1g_sp/$graph \
     $audio_file && sucesfully_decoded=true
else
  bash ~/saigen/tools/speech_recognition/decode_chain_callbi.sh \
    --diarization 0 \
    --sr $SR \
    --num-channels $NUM_CHAN \
    --num-threads $NUM_THREADS \
...
python3 $dir_helper_scripts/http_post_app.py \
-s3_decoded_path="$dest_path" \
-decoded_file_path="$dir_work/$TAG.json" \
-user_id="$USER_ID" \
-job_id="$JOB_ID" \
-x_api_key="$FE_API_KEY" \
-front_end_url="$RETURN_URL"
