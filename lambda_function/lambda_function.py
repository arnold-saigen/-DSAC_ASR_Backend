...
def lambda_handler(event, context):
    ...
    ses_recipient_list = ['example@example.com']
    ...
    optional_header_dict = {'CUSTOM_GRAPH': 'none', 'PRIV': 'yes', 'TAG': uniq_tag,
                            'OBJECT_URL': 'none', 'NUM_SPK': 1, 'CAP_PROV': 'FARGATE_SPOT',
                             'FE_API_KEY': 'none', 'JOB_ID': 'none', 'NUM_THREADS' : 4, 
                             'RETURN_URL' : 'https://saigendac.com/jobcomplete/'}
    ...
    optional_headers = ['CUSTOM_GRAPH', 'PRIV', 'TAG', 'OBJECT_URL', 'NUM_SPK', 
                        'CAP_PROV', 'FE_API_KEY', 'JOB_ID', 'NUM_THREADS', 'RETURN_URL']
    header_vals = {}
    ...
    response = client.run_task(
        ...
        taskDefinition='task_name:1', # replace with your task name
        count = 1,
        platformVersion='1.4.0',
        networkConfiguration={
            'awsvpcConfiguration': {
                'subnets': [
                    'subnet-1', # replace with your subnet ID's
                    'subnet-2',
                    'subnet-3'
                ],
                'securityGroups': [
                    'sg-0000xxxyyyzzz' # replace with your security group see README
                ],
